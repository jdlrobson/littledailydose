...
function lookupKey(char) {
    return Object.keys(vocabIndex).filter((key) => {
        return vocabIndex[key] === char;
    })[0];
}

function fixKeyInWord(newWord) {
    const key = lookupKey(newWord.char);
    if ( key ) {
...
module.exports = { extractFromHeading, extractDefinitions, makeLists,
    markReferenceLinks, boldFirstLine, parse, vocabIndex, lookupKey,
    reset, toMarkdown };
